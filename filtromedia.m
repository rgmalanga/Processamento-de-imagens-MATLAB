...
w= [1 1 1; 1 1 1; 1 1 1]; #filtro passa baixa, suaviza bordas
[nlin, ncol] = size(x);
[wnlin, wncol] = size(w);
g= zeros(nlin,ncol);
p= double(p);
...
g= uint8(round(g/9)); #se for filtro de media, tem q dividir por 9 (matriz 3x3)
z= imadd(x,g);
figure;
subplot(1,3,2);imshow(g);title("imagem filtrada");
subplot(1,3,3);imshow(z);title("bordas suavizadas") 
